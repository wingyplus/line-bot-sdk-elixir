# SPDX-License-Identifier: Apache-2.0
# SPDX-FileCopyrightText: 2026 Thanabodee Charoenpiriyakij

# NOTE: This file is auto generated by OpenAPI Generator unset (https://openapi-generator.tech).
# Do not edit this file manually.

defmodule LINE.Bot.Liff.Model.UpdateLiffAppRequest do
  @moduledoc """

  """

  @derive Jason.Encoder
  @derive JSON.Encoder
  defstruct [
    :view,
    :description,
    :features,
    :permanentLinkPattern,
    :scope,
    :botPrompt
  ]

  @type t :: %__MODULE__{
          :view => LINE.Bot.Model.UpdateLiffView.t() | nil,
          :description => String.t() | nil,
          :features => LINE.Bot.Model.LiffFeatures.t() | nil,
          :permanentLinkPattern => String.t() | nil,
          :scope => [LINE.Bot.Model.LiffScope.t()] | nil,
          :botPrompt => LINE.Bot.Model.LiffBotPrompt.t() | nil
        }

  alias LINE.Bot.Deserializer

  def decode(value) when is_map(value) and not is_struct(value) do
    Deserializer.raw_to_struct(value, __MODULE__)
  end

  def decode(value) do
    value
    |> Deserializer.deserialize(:view, :struct, LINE.Bot.Liff.Model.UpdateLiffView)
    |> Deserializer.deserialize(:features, :struct, LINE.Bot.Liff.Model.LiffFeatures)
    |> Deserializer.deserialize(:scope, :list, LINE.Bot.Liff.Model.LiffScope)
    |> Deserializer.deserialize(:botPrompt, :struct, LINE.Bot.Liff.Model.LiffBotPrompt)
  end
end
