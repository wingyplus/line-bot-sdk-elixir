# SPDX-License-Identifier: Apache-2.0
# SPDX-FileCopyrightText: 2026 Thanabodee Charoenpiriyakij

# NOTE: This file is auto generated by OpenAPI Generator unset (https://openapi-generator.tech).
# Do not edit this file manually.

defmodule LINE.Bot.MessagingApi.Model.CarouselTemplate do
  @moduledoc """

  """

  @derive Jason.Encoder
  @derive JSON.Encoder
  defstruct [
    :type,
    :columns,
    :imageAspectRatio,
    :imageSize
  ]

  @type t :: %__MODULE__{
          :type => String.t(),
          :columns => [LINE.Bot.Model.CarouselColumn.t()],
          :imageAspectRatio => String.t() | nil,
          :imageSize => String.t() | nil
        }

  alias LINE.Bot.Deserializer

  def decode(value) do
    value
    |> Deserializer.deserialize(:columns, :list, LINE.Bot.MessagingApi.Model.CarouselColumn)
  end
end
