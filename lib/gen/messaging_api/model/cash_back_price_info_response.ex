# SPDX-License-Identifier: Apache-2.0
# SPDX-FileCopyrightText: 2026 Thanabodee Charoenpiriyakij

# NOTE: This file is auto generated by OpenAPI Generator unset (https://openapi-generator.tech).
# Do not edit this file manually.

defmodule LINE.Bot.MessagingApi.Model.CashBackPriceInfoResponse do
  @moduledoc """

  """

  @derive Jason.Encoder
  @derive JSON.Encoder
  defstruct [
    :type
  ]

  @type t :: %__MODULE__{
          :type => String.t()
        }

  def decode(value) do
    value
  end

  def from_json(value) do
    case Map.get(value, "type") do
      "fixed" ->
        LINE.Bot.Deserializer.raw_to_struct(
          value,
          LINE.Bot.MessagingApi.Model.CashBackFixedPriceInfoResponse
        )

      "percentage" ->
        LINE.Bot.Deserializer.raw_to_struct(
          value,
          LINE.Bot.MessagingApi.Model.CashBackPercentagePriceInfoResponse
        )

      _ ->
        LINE.Bot.Deserializer.raw_to_struct(value, __MODULE__)
    end
  end
end
