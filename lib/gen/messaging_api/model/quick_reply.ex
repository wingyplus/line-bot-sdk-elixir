# SPDX-License-Identifier: Apache-2.0
# SPDX-FileCopyrightText: 2026 Thanabodee Charoenpiriyakij

# NOTE: This file is auto generated by OpenAPI Generator unset (https://openapi-generator.tech).
# Do not edit this file manually.

defmodule LINE.Bot.MessagingApi.Model.QuickReply do
  @moduledoc """
  Quick reply
  """

  @derive Jason.Encoder
  @derive JSON.Encoder
  defstruct [
    :items
  ]

  @type t :: %__MODULE__{
          :items => [LINE.Bot.Model.QuickReplyItem.t()] | nil
        }

  alias LINE.Bot.Deserializer

  def decode(value) when is_map(value) and not is_struct(value) do
    Deserializer.raw_to_struct(value, __MODULE__)
  end

  def decode(value) do
    value
    |> Deserializer.deserialize(:items, :list, LINE.Bot.MessagingApi.Model.QuickReplyItem)
  end
end
