# SPDX-License-Identifier: Apache-2.0
# SPDX-FileCopyrightText: 2026 Thanabodee Charoenpiriyakij

# NOTE: This file is auto generated by OpenAPI Generator unset (https://openapi-generator.tech).
# Do not edit this file manually.

defmodule LINE.Bot.Webhook.Model.PostbackEvent do
  @moduledoc """
  Event object for when a user performs a postback action which initiates a postback. You can reply to postback events.
  """

  @derive Jason.Encoder
  @derive JSON.Encoder
  defstruct [
    :type,
    :source,
    :timestamp,
    :mode,
    :webhookEventId,
    :deliveryContext,
    :replyToken,
    :postback
  ]

  @type t :: %__MODULE__{
          :type => String.t(),
          :source => LINE.Bot.Model.Source.t() | nil,
          :timestamp => integer(),
          :mode => LINE.Bot.Model.EventMode.t(),
          :webhookEventId => String.t(),
          :deliveryContext => LINE.Bot.Model.DeliveryContext.t(),
          :replyToken => String.t() | nil,
          :postback => LINE.Bot.Model.PostbackContent.t()
        }

  alias LINE.Bot.Deserializer

  def decode(value) do
    value
    |> Deserializer.deserialize(:source, :struct, LINE.Bot.Webhook.Model.Source)
    |> Deserializer.deserialize(:mode, :struct, LINE.Bot.Webhook.Model.EventMode)
    |> Deserializer.deserialize(:deliveryContext, :struct, LINE.Bot.Webhook.Model.DeliveryContext)
    |> Deserializer.deserialize(:postback, :struct, LINE.Bot.Webhook.Model.PostbackContent)
  end
end
