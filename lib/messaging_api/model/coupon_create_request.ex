# NOTE: This file is auto generated by OpenAPI Generator unset (https://openapi-generator.tech).
# Do not edit this file manually.

defmodule LINEMessagingAPI.Model.CouponCreateRequest do
  @moduledoc """
  Request object for creating a coupon. Contains all configurable coupon properties.
  """

  @derive JSON.Encoder
  defstruct [
    :acquisitionCondition,
    :barcodeImageUrl,
    :couponCode,
    :description,
    :endTimestamp,
    :imageUrl,
    :maxUseCountPerTicket,
    :startTimestamp,
    :title,
    :usageCondition,
    :reward,
    :visibility,
    :timezone
  ]

  @type t :: %__MODULE__{
    :acquisitionCondition => LINEMessagingAPI.Model.AcquisitionConditionRequest.t,
    :barcodeImageUrl => String.t | nil,
    :couponCode => String.t | nil,
    :description => String.t | nil,
    :endTimestamp => integer(),
    :imageUrl => String.t | nil,
    :maxUseCountPerTicket => integer(),
    :startTimestamp => integer(),
    :title => String.t,
    :usageCondition => String.t | nil,
    :reward => LINEMessagingAPI.Model.CouponRewardRequest.t | nil,
    :visibility => String.t,
    :timezone => String.t
  }

  def decode(value) do
    value
  end
end

